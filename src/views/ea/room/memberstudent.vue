<template>
  <basic-container>
    ...
    <el-dialog title="导入" :visible.sync="dialogFormVisible">
      <el-upload
          class="upload-demo"
          action="/api/ea/memberstudent/importExcel"
          multiple
          :headers="headers"
          :limit="1"
...
<script>
  ...
  import {getList, getDetail, add, update, remove,addClassRoomMember,memberList} from "@/api/ea/member/memberstudent";
  ...
  export default {
    ...
    computed: {
      ...
      permissionList() {
        return {
          addBtn: this.vaildData(this.permission.memberstudent_add, false),
          viewBtn: this.vaildData(this.permission.memberstudent_view, false),
          delBtn: this.vaildData(this.permission.memberstudent_delete, false),
          editBtn: this.vaildData(this.permission.memberstudent_edit, false)
        };
      },
      ids() {
        let ids = [];
    ...
    methods: {
      ...
      addTeaMember() {
        ...
        this.$confirm("确定要添加数据吗?", {
          ...
          .then(() => {
            addClassRoomMember(this.ids,this.classId,"2").then(res => {
